...
int test() {
  ...
  CHECK_GT(FLAGS_img.size(), 0);

  int name_index = -1;
  for (name_index = FLAGS_img.size() - 1; name_index >= 0; name_index--) {
      if (FLAGS_img[name_index] == '/') {
          break;
      }
  }
  std::string img_path = "";
  ...
  for (int i = 0; i <= name_index; i++) {
      img_path = img_path + FLAGS_img[i];
  }
  for (int i = name_index + 1; i < FLAGS_img.size(); i++) {
      imgname += FLAGS_img[i];
  }

  list_vec.push_back(imgname);
  ...
  //for (int i = 5; i < 6; ++i) {
    ...
    for (int j = 0; j < mini_batch; j++) {
        std::string img_name = img_path + list_vec[img_ind % list_vec.size()];
    ...
    for (int j = 0; j < mini_batch; j++) {
        ...
        if (FLAGS_img_save_path.size() > 0)
        {
            for (int k = 0; k < res_vec[j].size(); k++) {
                ...
                xmin = (xmin > 0 ? xmin : 0);
                ymin = (ymin > 0 ? ymin : 0);
                xmax = (xmax < img_vec[j].cols - 1 ? xmax : img_vec[j].cols - 1);
                ymax = (ymax < img_vec[j].rows - 1 ? ymax : img_vec[j].rows - 1);
                ...
                int shift = 15;
                if (max_conf > 0.6) {
                    cv::putText(img_vec[j], str, cv::Point(xmin, ymin + shift),CV_FONT_HERSHEY_PLAIN, 1.0, cv::Scalar(255, 0, 0));
                }
                else {
                    cv::putText(img_vec[j], str, cv::Point(xmin, ymin + shift),CV_FONT_HERSHEY_PLAIN, 1.0, cv::Scalar(0, 0, 255));
                }
            }

            std::string save_name = FLAGS_img_save_path + "/" + img_names[j];
